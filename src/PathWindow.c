...
static void menu_draw_row(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *callback_context) {
    ...
    //APP_LOG(APP_LOG_LEVEL_INFO, "starting to draw");
    GColor line_color = lines[stations[path.nodes[path.size - cell_index->row - 1].st].line].color;
    if(path.size != 1) {
        if (cell_index->row == 0) {
    ...
    //APP_LOG(APP_LOG_LEVEL_INFO, "starting to draw cars");
    if(path.nodes[path.size - cell_index->row - 1].s_cars != NULL) {
    ...
    GRect strect;
    if(strlen_utf8(stations[path.nodes[path.size - cell_index->row - 1].st].name) <= 15) {
        strect = GRect(20, 22 - 13, bounds.size.w - 23, bounds.size.h - 22);
    } else {
        strect = GRect(20, 0, bounds.size.w - 23, bounds.size.h);
    }
    graphics_draw_text(ctx,
                       stations[path.nodes[path.size - cell_index->row - 1].st].name,
...
static void load(Window *win) {
    ...
    palette_normal[4].argb = 0b00000000;
